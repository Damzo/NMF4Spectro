...
def floatX(x):
    return torch.tensor(x, dtype=torch.float32) #np.asarray(x, dtype=theano.config.floatX)
...
def dropout(x, p=0):
    if p == 0:
        ...
        return x * torch.bernoulli(x.shape, p=p, dtype=torch.float) #rng.binomial(x.shape, p=p, dtype=theano.config.floatX)
