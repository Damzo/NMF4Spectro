...
def TriOPNMF(X, alpha, beta, m, n, k, maxiter):
    ...
    for i in range(1,maxiter+1):   ####for i=1:maxiter
        ...
        #XX_TU = np.dot(XX_T,U)
        UU_T = np.dot(U, U.T)
        num = np.dot(X,DV.T) + beta*U
        den = np.dot((np.dot(U,DV)),DV.T) + beta*(np.dot(UU_T,U))
        den[den < 1e-10] = 1e-10
        re = np.divide(num, den)###np.multiply is for elemen-twise multiplication of two matrices
        re = np.power(re,(1/4))
        U = np.multiply(U, re)
        ## Update V
        UD = np.dot(U,D)
        num = np.dot(UD.T, X) + alpha*V
        den = np.dot(UD.T, np.dot(UD, V)) + alpha * np.dot(np.dot(V, V.T), V)
        den[den < 1e-10] = 1e-10
        re = np.divide(num, den)
        re = np.power(re, (1/4)) #np.sqrt(reV)
        V = np.multiply(V, re)
        ## Update D
        num = np.dot((np.dot(U.T, X)), V.T)
        den = np.dot((np.dot(U.T,U)),(np.dot((np.dot(D,V)),V.T)))
        den[den < 1e-10] = 1e-10
        re = np.divide(num, den)
        re = np.sqrt(re)  #nthroot(reD,2);
        D = np.multiply(D, re)

    return U, V, D

def run_model(model, dataset, matImg, y, k_knn_list, k_list, alpha_list, beta_list, maxiter, maxiter_kmeans):
    ...
    for knn in k_knn_list: # Number of nearest neighbors
        ...
        for k in k_list: # low embedding dimensions
            ...
            for p in parameters:
                ...
                U, V, D = TriOPNMF(X, alpha, beta, m, n, k, maxiter)

                n_iteration = maxiter
                iterations.append(n_iteration)

                ## Reconstructed matix
                X_reconstructed = U @ (D @ V)
