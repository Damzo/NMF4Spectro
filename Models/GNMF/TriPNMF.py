...
def TriPNMF(X,XXX,P,S,alpha,beta,m,n,k,maxiter):
    ...
    for i in range(1,maxiter+1):   ####for i=1:maxiter
        ...
        XXU = np.dot(XXX,U)
        UU =  np.dot(U, U.T)
        numU = np.dot(X,DV.T) +  XXU + beta*U
        denU = np.dot((np.dot(U,DV)),DV.T) +np.dot(UU,XXU) + np.dot((np.dot(XXU,U.T)),U) + beta*(np.dot(UU,U))
        denU[denU < 1e-10] = 1e-10
        reU = np.divide(numU,denU)###np.multiply is for elemen-twise multiplication of two matrices
        reU = np.power(reU,(1/4))
        U = np.multiply(U,reU);
        ## Update V
        UD = np.dot(U,D)
        numV = np.dot(UD.T,X )+ alpha*(np.dot(V,S))
        denV = np.dot(UD.T,(np.dot(UD,V))) + alpha *(np.dot(V,P))
        denV[denV < 1e-10] = 1e-10
        reV = np.divide(numV, denV)
        reV = np.sqrt(reV)
        V =np.multiply(V,reV)
        ## Update D
        numD = np.dot((np.dot(U.T, X)), V.T)
        denD =np.dot((np.dot(U.T,U)),(np.dot((np.dot(D,V)),V.T)))
        denD[denD < 1e-10] = 1e-10
        reD = np.divide(numD,denD)
        reD =  np.sqrt(reD)  #nthroot(reD,2);
        D = np.multiply(D,reD)

    return U, D, V

def run_model(model, dataset, matImg, y, k_knn_list, k_list, alpha_list, beta_list, maxiter, maxiter_kmeans, att):
    ...
    for knn in k_knn_list: # Number of nearest neighbors
        ...
        for k in k_list: # low embedding dimensions
            ...
            for p in parameters:
                alpha = p[0]
                beta = p[1] ##<< Open a for loup here
                U, D, V = TriPNMF(X, XXX, P_, S_, alpha, beta, m, n, k, maxiter)
                #U, V, n_iteration = GNMF(X, W, D, _lambda, k, m, n, maxiter)
                n_iteration = maxiter
                iterations.append(n_iteration)

                ## Reconstructed matix
                X_reconstructed1 = U @ (D @ V) #U @ V.T ##add
                X_reconstructed2 = (U @ U.T) @ X

                ## Reconstruction Error
                a = np.linalg.norm(X - X_reconstructed1, 'fro')*att + np.linalg.norm(X - X_reconstructed2, 'fro')*(1 - att) ## add
                recon_reeor = (a)  # /n
