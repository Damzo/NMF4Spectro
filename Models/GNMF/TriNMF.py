...
def TriPNMF(X,m,n,k,maxiter):
    ...
    for i in range(1,maxiter+1):
        ...
        BC = np.dot(B,C)
        num = np.dot(X,BC.T)
        den = np.dot((np.dot(R,BC)),BC.T)
        den[den < 1e-10] = 1e-10
        R = R * np.divide(num, den)
        ## Update B
        num = (R.T @ X) @ C.T
        den = ((R.T @ R) @ BC) @ C.T
        den[den < 1e-10] = 1e-10
        B = B * np.divide(num, den)
        ## Update D
        RB = R @ B
        num = RB.T @ X
        den = (RB.T @ RB) @ C
        den[den < 1e-10] = 1e-10
        C = C * np.divide(num, den)

    return R, B, C

def run_model(model, dataset, matImg, y, k_knn_list, k_list, maxiter, maxiter_kmeans):
    ...
    for knn in k_knn_list: # Number of nearest neighbors
        ...
        for k in k_list: # low embedding dimensions
            ...
            for p in parameters:
                R, B, C = TriPNMF(X, m, n, k, maxiter)
                n_iteration = maxiter
                iterations.append(n_iteration)

                ## Reconstructed matix
                X_reconstructed = (R @ B) @ C
